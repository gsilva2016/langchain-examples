...
from common.individual_agent.agent import root_agent
import os
...
async def adk_runner(args, last_processed_dt):
    ...
    user_id = "user"
    session_id = "session"
    app_name = "individual_summary_agent_app"
    initial_state = {
        "collection_name": args.report_collection_name,
    ...
    user_input = types.Content(
        role='user',
        parts=[types.Part(text="use individual_report_generator to store individual delivery agent metadata in Milvus database")]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report_collection_name", type=str, default="individual_report")
