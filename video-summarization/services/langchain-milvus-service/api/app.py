import argparse
from datetime import datetime
import sys
from typing import List
from fastapi import FastAPI, HTTPException
from pymilvus import Collection
import uvicorn
from pydantic import BaseModel
from pymilvus import connections
from pymilvus.orm import utility
from pymilvus import db, Collection
from langchain_community.embeddings.openvino import OpenVINOEmbeddings
from langchain_milvus import Milvus
from langchain_core.documents import Document
from pymilvus import Collection, connections
from langchain_openvino_clip import OpenVINOClipEmbeddings
import numpy as np


app = FastAPI()
txt_vectorstore = None

class VideoChunk(BaseModel):
    camera_id: int
    chunk_id: str
    chunk_path: str
    chunk_summary: str
    start_time: str
    end_time: str

class IngestTxtRequest(BaseModel):
    data: List[VideoChunk]

@app.get("/")
def root():
    """
    Root path for the application
    """
    return {"message": "Hello from App"}    

@app.post("/embed_txt_and_store")
async def embed_txt_and_store(request: IngestTxtRequest):
    try:
        documents = [
            Document(
                page_content=item.chunk_summary,
                metadata={
                    "camera_id": item.camera_id,
                    "chunk_id": item.chunk_id,
                    "start_time": item.start_time,
                    "end_time": item.end_time,
                    "chunk_path": item.chunk_path,
                    "timestamp": datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
                },
            )
            for item in request.data
        ]

        ids = [f"{doc.metadata['camera_id']}_{doc.metadata['chunk_id']}" for doc in documents]

        txt_vectorstore.add_documents(documents=documents, ids=ids)

        return {"status": "success", "total_chunks": len(documents)}

    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/archive_to_gcs")
async def archive_to_gcs(vector: List[str]):
    pass

@app.get("/query")
async def query(expr: str, collection_name: str="chunk_summaries"):
    try:
        collection = Collection(collection_name)  
        collection.load()
        
        results = collection.query(expr, collection_name=collection_name, output_fields=["chunk_id", "chunk_path", "camera_id"])
        print(f"{len(results)} vectors returned for query: {expr}")    
        
        return {"status": "success", "chunks": results}

    except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))

@app.get("/search")
async def search(query: str, top_k: int = 1):
    try:
        results = txt_vectorstore.similarity_search(
            query=query,
            k=top_k,
            filter=None,
            include=["metadata"],
        )

        return {
            "status": "success",
            "results": [
                {
                    "camera_id": doc.metadata["camera_id"],
                    "chunk_id": doc.metadata["chunk_id"],
                    "start_time": doc.metadata["start_time"],
                    "end_time": doc.metadata["end_time"],
                    "chunk_path": doc.metadata["chunk_path"],
                    "chunk_summary": doc.page_content
                }
                for doc in results
            ],
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

if __name__ == "__main__":

    parser = argparse.ArgumentParser()    
    parser.add_argument("--milvus_uri", default="localhost", type=str)
    parser.add_argument("--milvus_port", default=19530, type=int)
    parser.add_argument("--milvus_dbname", default="milvus_db", type=str)
    parser.add_argument("--txt_embedding_model", default="sentence-transformers/all-mpnet-base-v2", type=str)
    parser.add_argument("--txt_embedding_model_device", default="GPU", type=str)
    parser.add_argument("--img_embedding_model", default="openai/clip-vit-base-patch32", type=str)
    parser.add_argument("--img_embedding_model_device", default="GPU", type=str)
    parser.add_argument("--endpoint_uri", nargs="?", default="0.0.0.0")
    parser.add_argument("--endpoint_port", default=8001, type=int)
    args = parser.parse_args()

    if not args.endpoint_uri == "" and not args.endpoint_port:
        print("FastAPI server running on localhost:8001")
        
    try:
        model_name = args.txt_embedding_model
        model_kwargs = {"device": args.txt_embedding_model_device}
        encode_kwargs = {"mean_pooling": True, "normalize_embeddings": True}

        ov_txt_embeddings = OpenVINOEmbeddings(
            model_name_or_path=model_name,
            model_kwargs=model_kwargs,
            encode_kwargs=encode_kwargs)
                
        # Connect to Milvus
        conn = connections.connect(host=args.milvus_uri, port=args.milvus_port)
        if args.milvus_dbname not in db.list_database():
            db.create_database(args.milvus_dbname)
        db.using_database(args.milvus_dbname)
        
        collections = utility.list_collections()
        for name in collections:
            # Collection(name).drop()
            print(f"Collection {name} exists.")
        
        # Create Milvus instance
        txt_vectorstore = Milvus(
            embedding_function=ov_txt_embeddings,
            collection_name="chunk_summaries",
            connection_args={"uri": f"http://{args.milvus_uri}:{args.milvus_port}", "db_name": args.milvus_dbname},
            index_params={"index_type": "FLAT", "metric_type": "L2"},
            consistency_level="Strong",
            drop_old=False,
        )
        
        print("Connected to Milvus DB successfully.")
        
    except Exception as e:
        print(f"Error connecting to Milvus DB: {e}")
        sys.exit(1)
        
    uvicorn.run(app, host=args.endpoint_uri, port=args.endpoint_port)
